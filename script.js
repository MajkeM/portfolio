document.addEventListener("DOMContentLoaded", function () {
  ...
  if (hamburgerBtn && navLinkss) {
    ...
    document
      .querySelectorAll(".nav-links a, .nav-links button")
      .forEach((link) => {
  ...
  if (form) {
    ...
    form.addEventListener("submit", function (e) {
      ...
      if (!nameRegex.test(nameInput.value.trim())) {
        document.getElementById("nameError").textContent =
          "Jméno může obsahovat pouze písmena a mezery.";
        isValid = false;
      }
      ...
      if (!emailRegex.test(emailInput.value.trim())) {
        document.getElementById("emailError").textContent =
          "Prosím, zadejte platný formát e-mailu.";
        isValid = false;
      }
      ...
      if (messageInput.value.trim().length < 10) {
        ...
        document.getElementById("messageError").textContent =
          "Zpráva obsahuje nepovolené znaky (<, >).";
        isValid = false;
      }
      ...
      emailjs
        .sendForm("service_z65rmd3", "template_7dicc4u", this)
        .then(
          function (response) {
